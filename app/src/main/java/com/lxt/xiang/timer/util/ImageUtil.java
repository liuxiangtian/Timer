...
public class ImageUtil {
    ...
    public static void loadBitmap(final ImageView imageView, final long albumId){
        Observable.just(queryArtById(albumId))
    ...
    public static void loadBitmapBtArtistId(final ImageView albumArt, final long artistId) {
        Observable.just(ArtistLoader.loadAlbumIdByArtist(albumArt.getContext(), artistId))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public static void loadBitmapBtArtistId(final ImageView albumArt, final long artistId, final Palette.PaletteAsyncListener listener) {
        Observable.just(ArtistLoader.loadAlbumIdByArtist(albumArt.getContext(), artistId))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<Long>() {
                    @Override
                    public void call(Long albumId) {
                        loadBlurBitmap(albumArt, albumId);
//                        loadBitmap(albumArt, albumId, listener);
                    }
                });
    }

    public static void loadBitmapByPlaylistId(final ImageView albumArt, final long id, final Palette.PaletteAsyncListener paletteAsyncListener) {
        Observable.just(PlaylistLoader.getAlbumIdByPlaylistId(albumArt.getContext(), id))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<Long>() {
                    @Override
                        ...
                        Picasso.with(albumArt.getContext()).load(uri)
                                ...
                                .into(new SimpleTarget(){
                                    ...
                                    public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                                        ...
                                        Observable.just(createBlurredImageFromBitmap(bitmap,albumArt.getContext(),6 ))
                                                .subscribeOn(Schedulers.io())
                                                .observeOn(AndroidSchedulers.mainThread())
